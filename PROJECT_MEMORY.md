...
## E-L4-2B formal three-seed result (2026-08-02)
The final aligned experiment completed 60 trained tasks: M0 public DCRNN, M1 DSTSGCN traffic-only, M2 frozen-L4 auxiliary supervision and M3 M2 plus preregistered CVaR, across Bridge flow, Rainstorm flow/speed and the fixed 16-node Typhoon flow/speed task for seeds 42, 2024 and 3407. Every run used 20 epochs, history/horizon 12, the profile-compatible event-external split, train-only scaling and the dual-space missing-value contract. Bridge remained flow-only and event/weather features remained excluded.

A final-analysis audit found and corrected an evaluation inconsistency before accepting results: M2/M3 auxiliary-head archives had initially been used as their final L4 predictions while M0/M1 used frozen-profile postprocessing. The definitive analysis now applies the same rule to every model: physical traffic forecasts are transformed by the read-only frozen profile, `D_pred = g(Q_hat)` or `g(V_hat)`. Auxiliary-head archives are training diagnostics only.

"""D-STSGCN backbone components for traffic-resilience experiments.

The active thesis direction is a traffic-resilience model built on this
backbone. The older DGCN/quality_v2/OOD-scaled variants are retained here only
as reusable components and legacy internal baselines; they are not active
mainline models in ``run_experiments.py`` unless explicitly enabled there.
"""
...
class DSTSGCN(nn.Module):
    ...
    def __init__(
        ...
        ood_gamma_max: float = 0.35,
        resilience_aux: bool = False,
        resilience_target_mode: str = "ratio",
        resilience_uncertainty_weighting: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.resilience_aux = bool(resilience_aux)
        if resilience_target_mode not in {"ratio", "log_ratio"}:
            raise ValueError("resilience_target_mode must be ratio or log_ratio.")
        self.resilience_target_mode = resilience_target_mode
        self.resilience_uncertainty_weighting = bool(resilience_uncertainty_weighting)
        if self.resilience_uncertainty_weighting:
            self.traffic_log_var = nn.Parameter(torch.tensor(0.0))
            self.resilience_log_var = nn.Parameter(torch.tensor(0.0))
            self.consistency_log_var = nn.Parameter(torch.tensor(0.0))
        else:
            self.register_parameter("traffic_log_var", None)
            self.register_parameter("resilience_log_var", None)
            self.register_parameter("consistency_log_var", None)
        ...
        self.resilience_output = (
            nn.Sequential(
                nn.Linear(hidden_dim, hidden_dim),
                nn.ReLU(),
                nn.Linear(hidden_dim, horizon * output_dim),
            )
            if self.resilience_aux
            else None
        )
    ...
    def forward(
        ...
        final_state = h[:, -1]
        pred = self.output(final_state)
        pred = pred.view(x.size(0), self.num_nodes, self.horizon, self.output_dim)
        pred = pred.permute(0, 2, 1, 3).contiguous()
        resilience_pred = None
        if self.resilience_output is not None:
            resilience_pred = self.resilience_output(final_state)
            resilience_pred = resilience_pred.view(x.size(0), self.num_nodes, self.horizon, self.output_dim)
            resilience_pred = resilience_pred.permute(0, 2, 1, 3).contiguous()
            if self.resilience_target_mode == "ratio":
                resilience_pred = 2.0 * torch.sigmoid(resilience_pred)
        if not return_aux:
            return pred
        aux = {
        ...
        if resilience_pred is not None:
            aux["resilience_pred"] = resilience_pred
            if self.resilience_target_mode == "log_ratio":
                aux["resilience_log_ratio_pred"] = resilience_pred
                aux["resilience_deficit_pred"] = F.relu(-resilience_pred)
        if self.resilience_uncertainty_weighting:
            aux["traffic_log_var"] = torch.clamp(self.traffic_log_var, -5.0, 5.0)
            aux["resilience_log_var"] = torch.clamp(self.resilience_log_var, -5.0, 5.0)
            aux["consistency_log_var"] = torch.clamp(self.consistency_log_var, -5.0, 5.0)
        return pred, aux
